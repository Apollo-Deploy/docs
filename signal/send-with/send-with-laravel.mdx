...
description: "Send a transactional email from Laravel using the HTTP client"
---
...
Add `SIGNAL_API_KEY` to the server environment and expose it through a server-only config value.
Do not read the key from a request or return it to the browser.
  </Step>
  <Step title="Send the message">
```php
use Illuminate\Support\Facades\Http;

$response = Http::withToken(config('services.signal.api_key'))
    ->acceptJson()
