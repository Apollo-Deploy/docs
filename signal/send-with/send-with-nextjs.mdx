...
description: "Send a transactional email from a Next.js route handler"
---
...
Open **Domains**, add the sending domain, publish the required DNS records, and wait for the domain to show as verified.
  ...
  <Step title="Create a server-only route">
Add `SIGNAL_API_KEY` to your server environment.
Call Apollo Signal from a Route Handler or Server Action, never from a Client Component.
  </Step>
...
export async function POST(request: Request) {
  ...
  const response = await fetch("https://api.signal.apollodeploy.com/v1/emails", {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      ...
      text: "Use the secure sign-in link in your account.",
    }),
  });
