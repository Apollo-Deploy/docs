...
description: "Send a transactional email from Elixir using Req"
---
...
Open **Domains**, add the sending domain, publish the required DNS records, and wait for the domain to show as verified.
  ...
  <Step title="Install and configure Req">
Add `{:req, "~> 0.5"}` to your dependencies, then set `SIGNAL_API_KEY` in the process environment.
  </Step>
  <Step title="Send the message">
```elixir
Req.post!("https://api.signal.apollodeploy.com/v1/emails",
  auth: {:bearer, System.fetch_env!("SIGNAL_API_KEY")},
  json: %{
