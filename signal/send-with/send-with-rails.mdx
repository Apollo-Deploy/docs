...
description: "Send a transactional email from a Rails service"
---
...
Store `SIGNAL_API_KEY` in Rails credentials or your deployment secret manager.
Keep the call in a server-side service or background job.
  </Step>
...
class SignalEmail
  ENDPOINT = URI("https://api.signal.apollodeploy.com/v1/emails")

  def self.send_reset(user)
    request = Net::HTTP::Post.new(ENDPOINT)
    request["Authorization"] = "Bearer #{ENV.fetch("SIGNAL_API_KEY")}"
    ...
    Net::HTTP.start(ENDPOINT.hostname, 443, use_ssl: true) do |http|
