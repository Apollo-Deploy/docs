...
description: "Send a transactional email from Go using the standard HTTP client"
---
...
Set `SIGNAL_API_KEY` in the environment available to your Go service.
  </Step>
...
request, err := http.NewRequest(http.MethodPost, "https://api.signal.apollodeploy.com/v1/emails", bytes.NewReader(payload))
if err != nil {
    log.Fatal(err)
}

request.Header.Set("Authorization", "Bearer "+os.Getenv("SIGNAL_API_KEY"))
...
if response.StatusCode < 200 || response.StatusCode >= 300 {
    log.Fatalf("Apollo Signal returned %s", response.Status)
}
```
  </Step>
</Steps>
